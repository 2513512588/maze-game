...
class GamePerspective extends Game{
    ...
    //     // 通过anime方法一直进行碰撞检测
    //     this.addTask(() => {
    //         this.spirit.forEach((sphere, index) => {
    //             if (!this.gameOver && sphere.dir !== 0) {
    //                 let hitObj = sphere.hitTest(this.obstacles, this.spirit)
    //                 if (hitObj) {
    //                     //获取礼物
    //                     if (hitObj.obj.type === 2) {
    //                         this.scene.remove(hitObj.obj.target)
    //                         // hitObj.obj.target.dispose()
    //                         this.obstacles.splice(hitObj.index, 1)
    //                         let rd = random(0, 2)
    //                         if (rd === 0) {
    //                             this.generateSpirit(coverMapPos(this.MAP_POS_NAME, {
    //                                 z: hitObj.obj.target.position.z,
    //                                 x: hitObj.obj.target.position.x,
    //                             }))
    //                             new Toast('获得游戏精灵 x1').show()
    //                         } else if (rd === 1) {
    //                             sphere.move(() => {
    //                                 this.moveThisFloor(sphere, index === 0 && this.mode === 1)
    //                                 // this.updateScene()
    //                             }, 1)
    //                             let value = 15
