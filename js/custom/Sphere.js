...
class Sphere {
    ...
    constructor(scene, skin, position) {
        if (!scene) {
            throw '请传入场景对象'
        }
        this.dir = 0
    ...
    updateSkin(scene) {
        ...
        this.tween.onStart(() => {
            this.moveOnStart && this.moveOnStart()
        })
    ...
    moveTween(map, nameSpace) {
        ...
        if (this.dir === 1 || this.dir === 2) {
            if (this.dir === 1) {
                for (let i = position.x; i >= nextPos.realPos.x; i--) {
                    this.tempMove.push({
                        x: i,
                        z: nextPos.realPos.z
                    })
            ...
            if (this.dir === 3) {
                for (let i = position.z; i >= nextPos.realPos.z; i--) {
                    this.tempMove.push({
                        x: nextPos.realPos.x,
                        z: i
                    })
        ...
        this.tween.to(nextPos.realPos, this.tempMove.length * this.speedBlock)
