...
class Game {
    ...
    constructor() {
        this.audio = new Audio('./media/music.mp3')
        this.audio.play()
    }
    ...
    startup(options = {}) {
         ...
         * @type {number}
         */
        this.mode = this.swithBtn[0].getAttribute('mode') === 'on' ?  1 : 2


        //修改皮肤提示框
        this.skinDialog = new Dialog('.skin-dialog', skin => {
            this.spirit.forEach(sphere => {
                this.GAME_RECORD.skin = skin
                sphere.updateSkin(skin, this.scene)
            })
        ...
        document.onkeydown = e => {
            if (!this.paused && !this.gameOver) {
                if (e.code === 'KeyA' || e.code === 'KeyD' || e.code === 'KeyW' || e.code === 'KeyS' || e.code === 'ArrowLeft' || e.code === 'ArrowRight' || e.code === 'ArrowUp' || e.code === 'ArrowDown') {
                    //等待所有目标移动完成
                    if (this.spirit.filter(item => item.dir !== config.SPIRIT.DIR.NONE).length === 0){
                        this.spirit.forEach(sphere => {
